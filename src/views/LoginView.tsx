    ...
    <div className="pwa-accordion">
      <button className="pwa-accordion__toggle" onClick={() => setOpen((o) => !o)}>
        <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
          <Smartphone size={ICON_SIZE} strokeWidth={1.75} />
          Install as app
        </span>
        <motion.span
            ...
            <div className="pwa-accordion__body">
              ...
              {isIOS ? (
                <ol className="install-steps">
                  ...
                  <li>Tap the <span className="install-inline-icon"><Share size={ICON_SIZE} strokeWidth={2} /></span> <strong>Share</strong> button at the bottom</li>
                  <li>Tap <strong>"Add to Home Screen"</strong></li>
                </ol>
              ) : isAndroid ? (
                <ol className="install-steps">
                  <li>Open in <strong>Chrome</strong></li>
                  <li>Tap <span className="install-inline-icon"><MoreVertical size={ICON_SIZE} strokeWidth={2} /></span> in the top right</li>
                  <li>Tap <strong>"Add to Home Screen"</strong></li>
                </ol>
              ) : (
                <ol className="install-steps">
                  <li>Look for <span className="install-inline-icon"><PlusSquare size={ICON_SIZE} strokeWidth={2} /></span> in your address bar</li>
      ...
      <motion.div className="wizard-step" variants={stagger} initial="hidden" animate="show">
        ...
        <motion.form variants={fade} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%' }}>
          <div className="auth-field">
            <Mail size={ICON_SIZE} strokeWidth={1.75} className="auth-field__icon" />
            <input
          ...
          <div className="auth-field">
            <Lock size={ICON_SIZE} strokeWidth={1.75} className="auth-field__icon" />
            <input
              className="auth-input auth-input--icon"
              type="password"
          ...
          <button className="btn-primary auth-submit" type="submit" disabled={loading !== null}>
            {loading === 'email'
              ? <Loader2 size={ICON_SIZE} strokeWidth={2} className="spin" />
              : <>{mode === 'signin' ? <LogIn size={ICON_SIZE} strokeWidth={2} /> : <UserPlus size={ICON_SIZE} strokeWidth={2} />}{mode === 'signin' ? 'Sign in' : 'Create account'}</>
            }
          </button>
        </motion.form>
