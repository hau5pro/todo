...
function makeMockSupabase(upsertData = []) {
 ...
 */
function makePullMockSupabase(tableDataMap = {}) {
    const gtSpies = {};
...
describe('pushPending', () => {
    ...
    it('upserts pending lists to Supabase', async () => {
        ...
        await pushPending(await getDB(), mockSupa, 'user-123');
        expect(mockSupa.from).toHaveBeenCalledWith('lists');
        expect(mockSupa._upsertMock).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ id: list.id, user_id: 'user-123' })]), expect.any(Object));
    });
    ...
    it('upserts pending tasks to Supabase', async () => {
        ...
        expect(mockSupa._upsertMock).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ id: task.id, user_id: 'user-123' })]), expect.any(Object));
    });
...
describe('pullFromSupabase', () => {
    ...
    it('merges remote records into IDB with pending_sync: false and without user_id', async () => {
        ...
        await pullFromSupabase(db, mockSupa);
...
describe('initialSync', () => {
    ...
    it('clears todo_last_sync from localStorage before pulling', async () => {
        ...
        let capturedLastSync = 'not-checked';
        ...
        const db = await getDB();
        await initialSync(db, { from: fromMock });
