...
- [x] **Streak loading** — `useHabits.ts` and `MyDayView.tsx` both independently fetch completions + calculate streaks for each habit — consolidate
- [x] **`getTodayString()`** — `new Date().toISOString().split('T')[0]` repeated in ~6 places — extract to a shared util
...
- [x] **`countPending` in `useSync.ts`** — fetches all records from all 3 stores just to count — reuse the already-filtered results from `pushPending` instead of a full re-scan
